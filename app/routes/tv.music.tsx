...
/**
 * `/tv/music` — Leaderboard de proyector (CLIENT-RENDER · Sprint V1.6.1).
 *
 *   Mismo fix que `/tv/dashboard`: el login del SPA vive en localStorage
 *   (sin cookie), así que el viejo loader server (auth por cookie) daba 401
 *   al admin/display logueado.  Ahora hidrata en cliente con `POST /api/tv`
 *   (Bearer).  Variante "solo Top": sin QR ni modo batalla.
 */
...
type Boot =
	...
	| { phase: "need_login" }
	| { phase: "restricted" }
	| { phase: "ready"; tenantId: string; eventId: string | null; tracks: TvTrack[] };
	...
	if (boot.phase === "need_login") {
		return (
			<Center>
				...
				<p className="text-zinc-400 mt-1">Entra con una cuenta de staff (display/admin) para proyectar el Top.</p>
			</Center>
		);
	}
